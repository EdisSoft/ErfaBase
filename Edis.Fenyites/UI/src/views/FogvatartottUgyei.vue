<template>
  <div>
    <div class="page-header py-10 px-15 d-flex align-items-center">
      ...
      >
        A lenti listában a fogvatartott teljes fegyelmi helyzete elérhető
      </span>
    </div>
...
export default {
  ...
  methods: {
    ...mapActions({}),
    OnIntezetValasztas() {
      this.GetFogvatartottFegyelmiUgyei(this.$route.query.fogvatartottId);
    },
    async GetFogvatartottFegyelmiUgyei(fogvatartottId) {
      try {
        let result = await apiService.GetFogvatartottFegyelmiUgyei({
          fogvatartottId,
        });
