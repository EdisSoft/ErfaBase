<template>
  <div class="h-p100">
    ...
    <div class="page">
      ...
      <fegyelmi-statisztika v-if="vanJogosultsaga"></fegyelmi-statisztika>
    </div>
    <footer
...
import { fegyelmiListStatusz } from '../../store/modules/fegyelmiugy';

export default {
  name: 'fenyites-layout',
  data: function () {
    return {
      currenturl: '',
      urlWithToken: null,
  ...
  methods: {
    async GetAdatok() {
      console.log({ fegyelmiListStatusz });
      if (fegyelmiListStatusz.inited || fegyelmiListStatusz.loading) {
        return;
      }
      fegyelmiListStatusz.loading = true;
      try {
        await apiService.GetFegyelmiUgyek();
      } catch (error) {
        console.error(error);
      }
      fegyelmiListStatusz.loading = false;
    },
  },
  watch: {
    '$route.params': {
